 ...
 */
export async function estimateSeats(organization, repository, octokit) {
    const since = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString(); // 90 days ago date
    const branches = new Set();
    const uniqueCommitters = new Set();
    const usersWithLicenceActive = new Set();
    const usersWithoutLicence = new Set();
    ...
    const branchesResponse = (await getPaginatedData("/repos/{owner}/{repo}/branches", {
        owner: organization,
        repo: repository,
        per_page: 100,
    ...
    for (const branch of branches) {
        const commitsResponse = (await getPaginatedData("/repos/{owner}/{repo}/commits", {
            owner: organization,
            repo: repository,
            sha: branch,
            per_page: 100,
    ...
    const usersWithLicenceActiveResponse = (await getPaginatedData("/orgs/{org}/settings/billing/advanced-security", {
        org: organization,
        per_page: 100,
        headers: {
            "X-GitHub-Api-Version": "2022-11-28",
...
//# sourceMappingURL=estimateSeats.js.map